...
describe('CAP-TEST-006 product interview packet and gate', () => {
  it('exports a bounded product interview packet within the upload budget', () => {
    ...
    expect(packet.interviewBoundary).toMatch(/Product definition/i)
    expect(packet.interviewBoundary).toMatch(/draft-first review/i)
    expect(packet.interviewBoundary).toMatch(/accept it or correct/i)
    expect(packet.interviewBoundary).toMatch(/Do not run a field-by-field questionnaire/i)
    expect(packet.interviewBoundary).toMatch(/at most 5 concise prompts/i)
