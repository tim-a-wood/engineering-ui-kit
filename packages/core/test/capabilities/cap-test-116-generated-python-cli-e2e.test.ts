...
describe('CAP-TEST-116 generated Python CLI target', () => {
  it('launches, dispatches through generated composition, and proves the observed path', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'euik-python-cli-generated-'))
    roots.push(root)
    ...
    const binding: CliInboundBinding = {
      ...
      deployableId: 'python-cli', operationId: operation.operationId, operationVersion: operation.version,
    ...
    const deployable: DeployableSpecification = {
      schemaVersion: '1.0', deployableId: 'python-cli', name: 'Python CLI', kind: 'cli', runtimeLanguage: 'python',
      runtimeVersionRange: '>=3.11', moduleIds: ['module.echo'], inboundBindingIds: [binding.bindingId],
      compositionRootPath: 'src/composition/python_cli.py', commands: {}, configurationRefs: [], secretReferenceIds: [], proposedLocations: [],
    }
    const compositionBody = {
      schemaVersion: '1.0' as const, projectId: 'project-1', compositionId: 'python-cli', applicationRevision: '1', architectureRevision: '1',
    ...
    const assembled = assembleGenerationPlan({
      ...
      planId: 'plan-python-cli-real', runId: 'run-python-cli-real',
    })
    ...
    expect(record.observedPath).toEqual({
      inboundAdapter: 'cli:binding.echo.cli', compositionRoot: 'src/composition/python_cli.py',
