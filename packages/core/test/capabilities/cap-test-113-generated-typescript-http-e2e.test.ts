...
describe('CAP-TEST-113 generated TypeScript HTTP target', () => {
  it('compiles, launches, dispatches through generated composition, and emits observed-path evidence', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'euik-ts-generated-'))
    roots.push(root)
    ...
    fs.writeFileSync(path.join(root, 'src/domain/echo.ts'), [
      ...
      '  return {',
      "    code: 'echo.run',",
      '    execute(input: { message: string }) { return Outcome.success({ echo: input.message }) },',
      '  }',
      '}',
      '',
    ].join('\n'))
    ...
    const binding: HttpInboundBinding = {
      ...
      deployableId: 'typescript-api', operationId: operation.operationId, operationVersion: operation.version,
      ...
      duplicateSubmissionBehavior: 'none', exposure: 'private', generatedTargets: [], approvalState: 'approved', method: 'POST', path: '/echo',
    }
    const deployable: DeployableSpecification = {
      schemaVersion: '1.0', deployableId: 'typescript-api', name: 'TypeScript API', kind: 'http-api', runtimeLanguage: 'typescript',
      runtimeVersionRange: '>=22', moduleIds: ['module.echo'], inboundBindingIds: [binding.bindingId],
      compositionRootPath: 'src/composition/typescript-api.ts', commands: {}, configurationRefs: [], secretReferenceIds: [], proposedLocations: [],
    }
    const compositionBody = {
      schemaVersion: '1.0' as const, projectId: 'project-1', compositionId: 'typescript-api', applicationRevision: '1', architectureRevision: '1',
      deployableIds: ['typescript-api'], registrations: [{
    ...
    const assembled = assembleGenerationPlan({
      ...
      planId: 'plan-ts-real', runId: 'run-ts-real',
    })
    applyGenerationPlan({ plan: assembled.plan, targetRoot: root, virtualFiles: assembled.virtualFiles, runId: 'run-ts-real' })
    ...
    const record = await runConnectionVerification({
      verificationId: 'verification-ts-real', projectId: 'project-1', binding, deployable, hashes,
      launch: { command: process.execPath, args: ['dist/src/generated/typescript-api/host.g.js'], cwd: root, healthPath: '/health', readyTimeoutMs: 10_000 },
      trigger: { kind: 'http', method: 'POST', path: '/echo', body: { message: 'hello' } },
      correlationId: 'correlation-ts-real',
    })
    expect(record.verificationStatus, JSON.stringify(record, null, 2)).toBe('pass')
    expect(record.observedPath).toEqual({
      inboundAdapter: 'http:binding.echo.http', compositionRoot: 'src/composition/typescript-api.ts',
