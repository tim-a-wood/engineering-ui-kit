...
import { checkSteText, type SteTextClass } from '../simplifiedTechnicalEnglish.js'
...
function evaluatePlanSte(analysis: UseCaseAnalysis): CapDiagnostic[] {
  const diagnostics: CapDiagnostic[] = []
  const check = (text: string, textClass: SteTextClass, fieldPath: string) => {
    diagnostics.push(...checkSteText(text, { textClass, fieldPath }).diagnostics)
  }

  analysis.actors.forEach((item) => check(item.text, 'technical-name', `actors.${item.id}.text`))

  for (const useCase of analysis.useCases) {
    const root = `useCases.${useCase.id}`
    check(useCase.name, 'action-label', `${root}.name`)
    for (const scenario of useCase.scenarios) {
      const scenarioRoot = `${root}.scenarios.${scenario.id}`
      check(scenario.name, 'action-label', `${scenarioRoot}.name`)
      scenario.steps.forEach((step) => {
        check(step.action, 'action-label', `${scenarioRoot}.steps.${step.id}.action`)
      })
    }
  }
  return diagnostics
...
export function evaluatePlanGate(analysis: UseCaseAnalysis): GateResult {
  ...
  diagnostics.push(...evaluatePlanSte(analysis))
