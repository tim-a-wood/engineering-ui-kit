...
function buildApplicationSpecification(analysis: UseCaseAnalysis): ApplicationSpecification {
  ...
  const compiled = compileApplication(analysis)
  if (!compiled.specification) {
    throw new Error(`sample application failed to compile: ${JSON.stringify(compiled.diagnostics)}`)
  }
  ...
  const draft: ApplicationSpecification = {
    ...
    actors: compiled.specification.actors,
    goals: [
    ...
    useCases: compiled.specification.useCases,
    scenarios: compiled.specification.scenarios,
    useCaseDefinitions: compiled.specification.useCaseDefinitions,
    applicationWorkflows: compiled.specification.applicationWorkflows,
    scenarioDefinitions: compiled.specification.scenarioDefinitions,
    information: [
    ...
    acceptanceCases: compiled.specification.acceptanceCases,
...
function refreshEvidenceUseCase(): UseCaseDefinition {
  ...
  const mainFlow: ScenarioStep[] = [
    step(ucId, 'select', 'Start evidence refresh', 'The system stages every configured source as a candidate.'),
    step(ucId, 'validate', 'Validate source candidate', 'Every valid candidate is published as a new snapshot.'),
    step(ucId, 'report', 'Report refresh status', 'The audit lead sees the updated readiness view.'),
  ]
  const alternate: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'cancel-refresh'),
    name: 'Cancel refresh',
    kind: 'alternate',
    steps: [step(ucId, 'cancel', 'Cancel evidence refresh', 'The system stops staging new candidates and keeps every already-published snapshot unchanged.')],
  }
  const failure: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'source-unreachable'),
    name: 'Check source access',
    kind: 'failure',
    steps: [
      step(
        ucId,
...
function reviewFindingUseCase(): UseCaseDefinition {
  ...
  const mainFlow: ScenarioStep[] = [
    step(ucId, 'open', 'Open finding', 'The system shows the finding, its evidence, and the identity of the finding author.'),
    step(ucId, 'decide', 'Submit review decision', 'The system records the decision because the reviewer is independent of the author.'),
    step(ucId, 'close', 'Close finding', 'The finding state changes to closed.'),
  ]
  const alternate: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'reopen-finding'),
    name: 'Reopen closed finding',
    kind: 'alternate',
    steps: [step(ucId, 'reopen', 'Reopen closed finding', 'The finding state changes back to open for a new decision.')],
  }
  const failure: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'not-independent'),
    name: 'Reject author decision',
    kind: 'failure',
    steps: [
      step(
        ucId,
        'reject',
        'Submit author decision',
  ...
  const recovery: UseCaseScenario = {
    ...
    name: 'Submit independent decision',
    kind: 'recovery',
    steps: [
      step(
        ucId,
        'independent-decide',
        'Submit independent decision',
...
function exportPackageUseCase(): UseCaseDefinition {
  ...
  const mainFlow: ScenarioStep[] = [
    step(ucId, 'select', 'Select approved baseline', 'The system confirms the baseline is approved and current.'),
    step(ucId, 'create', 'Create audit package', 'The system builds a deterministic package and manifest.'),
    step(ucId, 'status', 'Check package status', 'The system shows the completed package with its hash.'),
  ]
  const alternate: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'cancel-package'),
    name: 'Cancel package export',
    kind: 'alternate',
    steps: [step(ucId, 'cancel-package', 'Cancel package export', 'The system stops and discards the partial package.')],
  }
  const failure: UseCaseScenario = {
    ...
    kind: 'failure',
    steps: [
      step(ucId, 'missing', 'Request incomplete package', 'The system fails the export and reports which evidence is missing.'),
    ],
  }
  const recovery: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'retry-after-evidence-added'),
    name: 'Retry package export',
    kind: 'recovery',
    steps: [
      step(ucId, 'retry-package', 'Retry package export', 'The system builds the package successfully.'),
    ],
  }
  return {
    id: ucId,
    ...
    alternatePaths: [alternate],
    failurePaths: [failure],
    recoveryBehavior: 'The last successful package remains available. The audit lead can retry after adding the missing evidence.',
...
function browseLifecycleUseCase(): UseCaseDefinition {
  ...
  const mainFlow: ScenarioStep[] = [
    step(ucId, 'phase', 'Open lifecycle phase', 'The system shows evidence for that phase.'),
    step(ucId, 'view', 'View evidence item', 'The system shows the item and its known trace links.'),
    step(ucId, 'trace', 'Follow requirement trace', 'The system navigates to the linked model element.'),
  ]
  const alternate: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'compare-revisions'),
    name: 'Compare evidence revisions',
    kind: 'alternate',
    steps: [step(ucId, 'compare', 'Compare evidence revisions', 'The system shows the differences between the two revisions.')],
  }
  const failure: UseCaseScenario = {
    id: childId(ucId, 'scenario', 'broken-trace'),
    name: 'Report broken trace',
    kind: 'failure',
    steps: [
      step(
        ucId,
  ...
  const recovery: UseCaseScenario = {
    ...
    name: 'Open different trace',
    kind: 'recovery',
    steps: [
      step(
        ucId,
        'continue',
        'Open different trace',
  ...
  return {
    ...
    alternatePaths: [alternate],
    failurePaths: [failure],
    recoveryBehavior: 'The app reports a broken trace as a defect. The auditor can browse other traces.',
    rules: [],
...
function buildUseCaseAnalysis(): UseCaseAnalysis {
  const base: UseCaseAnalysis = {
    ...
    status: 'draft',
    workDescription:
      'Audit leads refresh evidence, review findings, export an audit package, and browse lifecycle evidence.',
    examples: [
...
const WORKFLOW_ACTION_ALLOCATIONS: Record<string, { primaryModuleId: string; operationId: string }> = {
  'Start evidence refresh': { primaryModuleId: MOD.importAndPublish, operationId: 'RefreshEvidence' },
  'Validate source candidate': { primaryModuleId: MOD.workspaceSnapshots, operationId: 'StartValidation' },
  'Report refresh status': { primaryModuleId: MOD.importAndPublish, operationId: 'GetRefreshStatus' },
  'Cancel evidence refresh': { primaryModuleId: MOD.importAndPublish, operationId: 'CancelRefresh' },
  'Check source access': { primaryModuleId: MOD.importAndPublish, operationId: 'GetRefreshStatus' },
  'Retry evidence refresh': { primaryModuleId: MOD.importAndPublish, operationId: 'RefreshEvidence' },
  'Open finding': { primaryModuleId: MOD.auditWorkspace, operationId: 'OpenFinding' },
  'Submit review decision': { primaryModuleId: MOD.findingReview, operationId: 'SubmitFindingDecision' },
  'Close finding': { primaryModuleId: MOD.findingReview, operationId: 'CloseFinding' },
  'Reopen closed finding': { primaryModuleId: MOD.findingReview, operationId: 'ReopenFinding' },
  'Submit author decision': { primaryModuleId: MOD.findingReview, operationId: 'SubmitFindingDecision' },
  'Submit independent decision': { primaryModuleId: MOD.findingReview, operationId: 'SubmitFindingDecision' },
  'Select approved baseline': { primaryModuleId: MOD.auditWorkspace, operationId: 'OpenReadiness' },
  'Create audit package': { primaryModuleId: MOD.packageExport, operationId: 'CreateAuditPackage' },
  'Check package status': { primaryModuleId: MOD.packageExport, operationId: 'GetPackageStatus' },
  'Cancel package export': { primaryModuleId: MOD.packageExport, operationId: 'CancelPackage' },
  'Request incomplete package': { primaryModuleId: MOD.packageExport, operationId: 'CreateAuditPackage' },
  'Retry package export': { primaryModuleId: MOD.packageExport, operationId: 'CreateAuditPackage' },
  'Open lifecycle phase': { primaryModuleId: MOD.lifecycleExplorer, operationId: 'OpenLifecyclePhase' },
  'View evidence item': { primaryModuleId: MOD.lifecycleExplorer, operationId: 'OpenLifecyclePhase' },
  'Follow requirement trace': { primaryModuleId: MOD.evidenceGraph, operationId: 'FollowTrace' },
  'Compare evidence revisions': { primaryModuleId: MOD.lifecycleExplorer, operationId: 'CompareEvidence' },
  'Follow broken trace': { primaryModuleId: MOD.evidenceGraph, operationId: 'FollowTrace' },
  'Open different trace': { primaryModuleId: MOD.lifecycleExplorer, operationId: 'OpenLifecyclePhase' },
}
...
function buildArchitecture(application: ApplicationSpecification): SystemStructureSpecification {
  ...
  const primaryModuleByUseCase = new Map<string, string>([
    [childId(ANALYSIS_ID, 'use-case', 'refresh-evidence'), MOD.importAndPublish],
    [childId(ANALYSIS_ID, 'use-case', 'review-finding'), MOD.findingReview],
    [childId(ANALYSIS_ID, 'use-case', 'export-package'), MOD.packageExport],
    [childId(ANALYSIS_ID, 'use-case', 'browse-lifecycle'), MOD.lifecycleExplorer],
  ])
  const workflowTraces = workflowTracesBase.map((trace) => {
    const primaryModuleId = primaryModuleByUseCase.get(trace.useCaseId) ?? trace.moduleIds[0]!
    return {
      ...trace,
      nodeAllocations: (application.applicationWorkflows ?? [])
        .filter((workflow) => workflow.useCaseId === trace.useCaseId)
        .flatMap((workflow) => workflow.graph.nodes
          .filter(isExecutableActivityNode)
          .map((node) => {
            const allocation = WORKFLOW_ACTION_ALLOCATIONS[node.label]
            return {
              workflowId: workflow.id,
              nodeId: node.id,
              primaryModuleId: allocation?.primaryModuleId ?? primaryModuleId,
              participatingModuleIds: [],
              ...(allocation?.operationId ? { operationId: allocation.operationId } : {}),
            }
          })),
    }
  })
...
export function buildSampleAuditHub(): SampleAuditHub {
  ...
  const applicationSpecification = buildApplicationSpecification(useCaseAnalysis)
  ...
  const filledDrafts: ModuleDesignSpecification[] = MODULE_DEFS.map((def) => {
    ...
    merged.behavior = {
      ...merged.behavior,
      activities: merged.behavior.activities?.map((activity, index) => ({
        ...activity,
        refinesWorkflowNodeIds: index === 0 ? [...(base.trace.workflowNodeIds ?? [])] : [],
      })),
    }
