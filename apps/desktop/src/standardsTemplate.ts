...
const STANDARD_BASE_TOKEN_ROWS: TaskDefinition['tokenRows'] = [
  { path: 'semantic.surface.canvas', value: '#0d1117', requiredUse: 'Root app canvas background' },
  { path: 'semantic.surface.panel', value: '#161b22', requiredUse: 'Primary bounded panels' },
  { path: 'semantic.surface.panelRaised', value: '#161b22', requiredUse: 'Standard component surface' },
  { path: 'semantic.surface.inset', value: '#0d1117', requiredUse: 'Code, plot, diagram, and log background' },
  { path: 'semantic.surface.overlay', value: '#1c2128', requiredUse: 'Dialog and popover surfaces' },
  { path: 'semantic.surface.scrim', value: 'rgba(2, 6, 23, 0.72)', requiredUse: 'Modal backdrop' },
  { path: 'semantic.text.primary', value: '#f0f6fc', requiredUse: 'Primary headings and body text' },
  { path: 'semantic.text.secondary', value: '#b1bac4', requiredUse: 'Supporting labels and metadata' },
  { path: 'semantic.text.muted', value: '#8b949e', requiredUse: 'Helper and step labels' },
  ...
  { path: 'semantic.border.subtle', value: '#30363d', requiredUse: 'Default panel and field borders' },
  { path: 'semantic.border.strong', value: '#5f6872', requiredUse: 'Stronger separators' },
  { path: 'semantic.border.focus', value: '#58a6ff', requiredUse: 'Focus outline color' },
  { path: 'semantic.border.danger', value: '#ef4444', requiredUse: 'Validation error borders' },
  { path: 'semantic.focus.ring', value: '0 0 0 2px #58a6ff', requiredUse: 'Visible focus ring' },
  { path: 'semantic.focus.ringOffset', value: '0 0 0 4px rgba(88, 166, 255, 0.18)', requiredUse: 'Focus ring separation' },
  { path: 'semantic.accent.foreground', value: '#58a6ff', requiredUse: 'Links, icons, and data emphasis' },
  { path: 'semantic.accent.primary', value: '#1f6feb', requiredUse: 'Primary action fill' },
  { path: 'semantic.accent.primaryHover', value: '#1a64d6', requiredUse: 'Primary action hover' },
  { path: 'semantic.accent.primaryActive', value: '#195bbf', requiredUse: 'Primary action pressed' },
  { path: 'semantic.accent.secondary', value: '#a985f5', requiredUse: 'Secondary data series only when needed' },
  { path: 'semantic.accent.muted', value: '#1b2a3a', requiredUse: 'Selected state background' },
  { path: 'semantic.accent.border', value: '#58a6ff', requiredUse: 'Selected state boundary' },
  ...
  { path: 'semantic.status.neutral', value: '#8b949e', requiredUse: 'Neutral status text' },
  ...
  { path: 'semantic.charts.grid', value: 'rgba(139, 148, 158, 0.16)', requiredUse: 'Chart gridlines at every major tick' },
  { path: 'semantic.charts.axis', value: '#8b949e', requiredUse: 'Axis lines, tick labels, and axis titles' },
  { path: 'semantic.charts.crosshair', value: 'rgba(177, 186, 196, 0.42)', requiredUse: 'Hover/keyboard crosshair rule and active-point halo' },
  { path: 'semantic.charts.series.primary', value: '#58a6ff', requiredUse: 'First data series' },
  { path: 'semantic.charts.series.secondary', value: '#a985f5', requiredUse: 'Second data series' },
...
function colorTokenRows(config: FrontendDesignSystemConfig): TaskDefinition['tokenRows'] {
  return (['light', 'dark'] as const).flatMap((modeName) => {
    ...
    return [
      ...
      { path: `semantic.mode.${modeName}.color.overlay`, value: mode.overlay, requiredUse: `${modeName} overlay surface` },
      ...
      { path: `semantic.mode.${modeName}.color.accentForeground`, value: mode.accentForeground, requiredUse: `${modeName} link, icon, and data emphasis` },
      { path: `semantic.mode.${modeName}.color.action`, value: mode.action, requiredUse: `${modeName} primary action fill` },
      { path: `semantic.mode.${modeName}.color.actionHover`, value: mode.actionHover, requiredUse: `${modeName} primary action hover` },
      { path: `semantic.mode.${modeName}.color.actionActive`, value: mode.actionActive, requiredUse: `${modeName} primary action press` },
      { path: `semantic.mode.${modeName}.color.onAccent`, value: mode.onAccent, requiredUse: `${modeName} text on primary action` },
      { path: `semantic.mode.${modeName}.color.control`, value: mode.control, requiredUse: `${modeName} neutral control fill` },
      { path: `semantic.mode.${modeName}.color.controlHover`, value: mode.controlHover, requiredUse: `${modeName} neutral control hover` },
      { path: `semantic.mode.${modeName}.color.selected`, value: mode.selected, requiredUse: `${modeName} selected state surface` },
      { path: `semantic.mode.${modeName}.color.selectedBorder`, value: mode.selectedBorder, requiredUse: `${modeName} selected state boundary` },
...
export const STANDARD_APPROVED_GUIDANCE = [
  ...
  'Concise page header.',
  'Natural application names and page titles that identify the product, task, or selected object.',
  ...
  'A metric surface with an explicit decision purpose; no measure appears unless it changes the current task decision.',
  ...
  'Workflow navigation matches complexity: paired modes use title-bar text transitions; only genuinely sequential flows with three or more gated phases use a compact step indicator.',
  'Project and artifact lists use full-row navigation with visible hover/focus; never add a redundant Continue button to an already-clickable row.',
  'Major workflow titles may use one unboxed semantic icon; do not wrap the icon in a decorative tile.',
  'Preview regions persist before implementation exists with an explicit Placeholder preview label, intended local URL, subdued mock shell, and retry action.',
  'File handoffs support browse, drag-in, native drag-out, and an equivalent copy action.',
...
export const STANDARD_REJECTED_GUIDANCE = [
  ...
  'A count-led page title or dramatic status slogan such as Three sessions wait.',
  'A default metric strip with no documented task decision.',
...
export const STANDARD_CONSTRAINTS = [
  ...
  'Start in dark mode. Use one near-black navy canvas and one subtly lighter component surface.',
  'Do not add more background tiers, blue panel bands, or decorative gradients.',
  'Use a subdued blue accent only for actions, focus, selection, and data emphasis.',
...
export const STANDARD_EXCERPTS: TaskDefinition['standardsExcerpts'] = [
  ...
  },
  {
    ...
    body: [
      ...
      'Status tiles are justified only for 3–5 true peer metrics that answer a standing engineering question and change a task decision. Record that purpose in the page specification. Otherwise integrate status into the working surface (table rows, badges, callouts). A chart never floats alone: it sits in a chart panel adjacent to the data table or summary that carries the same information as text. Numeric table cells right-align in tabular numerals; date and identifier cells never wrap mid-value.',
  ...
  {
    id: 'RCP-WORKFLOW-001',
    title: 'Workflow navigation proportional to complexity',
    body: [
      'Navigation complexity shall match workflow complexity. Two closely related modes such as Build and Test use clear page titles with one unboxed semantic icon and a quiet text-and-arrow transition in the page header; do not add a stepper, duplicate global controls, or segmented state selector.',
      'Use a compact step indicator only for three or more genuinely sequential phases where users benefit from visible completion state and cannot safely jump forward. Completed phases may be revisited; forward navigation remains validation-gated; Back never loses input.',
      'The final consequential action still requires an explicit review or summary. Navigation does not replace validation, status, preview, or confirmation.',
    ].join(' '),
