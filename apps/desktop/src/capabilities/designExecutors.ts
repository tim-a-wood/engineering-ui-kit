 ...
 * binding or run an approved scenario through a production entry point:  * only an injected test executor exercised that code
 * (`packages/core/test/capabilities/design/product-scenarios.test.ts`
 * S26/S27).
 *
 * The real implementation lives in `packages/core`
 * (`capabilities/design/connectExecutors.ts`, a Node adapter-layer module:  * see its module doc for exactly what each executor does and does not
...
import { createConnectExecutors, type ConnectExecutorDeps, type DesignOperationExecutors, type DesignWorkspace, type ModuleDesignSpecification } from '@engineering-ui-kit/core/design'
