    ...
    <Dialog title={`${label}: ${props.diagramTitle}`} onClose={props.onClose} wide>
      <section className="design-diagram-detail-summary">
        <div>
          ...
          <h3>{isElement ? definition || 'Definition not recorded' : `${connected[0]} · ${connected[1]}`}</h3>
        </div>
        <dl>
      ...
      <form className="design-diagram-propose-form" onSubmit={submitPropose}>
        <h3>Propose controlled change</h3>
        <label htmlFor={proposeFieldId}>{props.target.isRenameable ? 'New name' : 'Describe proposed change'}</label>
