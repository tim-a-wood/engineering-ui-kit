...
import type { ApplicationSpecification, OperationContract, CapDiagnostic } from '@engineering-ui-kit/core'
import {
  DesignBridgeClient,
...
type WorkflowStatusResult = {
  ...
  application: { draft?: ApplicationSpecification; approved?: ApplicationSpecification }
...
function loadDefaultSnapshot(): {
  ...
  return {
    ...
    application: sample.applicationSpecification,
...
function emptyApplicationSpecification(projectId: string): ApplicationSpecification {
  return {
    schemaVersion: '1.0',
    projectId,
    id: '',
...
function emptyProjectState(projectId: string, actor: string = LOCAL_USER_ACTOR): DesignState {
  ...
  return {
    ...
    projectId,
    syntheticDataStatement: '',
    application: emptyApplicationSpecification(projectId),
    architecture,
...
export class DesignStore {
  ...
  private async refreshProjectState(): Promise<void> {
    ...
    try {
      ...
      const application = workflowStatus.application?.approved ?? workflowStatus.application?.draft ?? emptyApplicationSpecification(client.projectId)
  ...
  async grantDesignAuthoritiesToSessionUser(): Promise<RolesGrantState> {
    ...
    if (principalState.status !== 'ready') {
      const result: RolesGrantState = { status: 'error', message: 'The session principal is not available yet. The app cannot grant authorities to an unknown user.' }
  ...
  approveUseCaseAnalysis(): void {
    if (!this.isProjectMode()) return
    const client = this.requireBridge()
    this.pendingOperation = this.runChangeOperation<{ analysis: UseCaseAnalysis; application?: ApplicationSpecification }>(
      ...
      (value) => this.patch({
        useCaseAnalysis: value.analysis,
  ...
  approveModule(moduleId: string, approvedBy = 'you'): { ok: boolean; diagnostics: readonly { message: string }[] } {
    ...
    if (this.isProjectMode()) {
      ...
      return { ok: false, diagnostics: [{ message: 'Approval was submitted to the service. See the module status after the service responds.' }] }
    }
    ...
    if (this.isProjectMode()) {
      ...
      if (!design || design.status !== 'approved') {
        const result: ModuleHandoffResult = {
          ok: false,
          kind: 'design',
          diagnostics: [{ code: 'CAP-DES-BUILD-NOT-APPROVED', message: 'This module design is not approved. Approve it before you create an implementation handoff.' }],
          manifest,
          createdAt: now,
        }
  ...
  createMultiModuleHandoff(moduleIds: string[], confirmations: MultiModuleConfirmations, limitBytes = 200_000): BuildMultiModulePacketResult {
    if (this.isProjectMode()) {
      ...
      const result: BuildMultiModulePacketResult = {
        ok: false,
        diagnostics: [{ code: 'CAP-DES-PKT-MULTI-UNAVAILABLE', message: 'Multi-module handoff is not available in project mode. Use the single-module implementation handoff.' }],
      }
  ...
  approveReturnedDelta(moduleId: string, approvedBy = 'you'): { ok: boolean; diagnostics: CapDiagnostic[] } {
    ...
    if (this.isProjectMode()) {
      ...
      return { ok: false, diagnostics: [{ code: 'CAP-DES-DELTA-PENDING', message: 'Approval was submitted to the service. See the delta flow status after the service responds.' }] }
    }
  ...
  rollbackReturnedDelta(moduleId: string): void {
    if (this.isProjectMode()) {
      ...
      this.announce('Manual rollback is not available in project mode. The service applies changes as one transaction.')
      return
    }
    const flow = this.deltaFlowFor(moduleId)
