  ...
  return (
    <article className="design-plan-use-case-detail" aria-label={`Use case ${useCase.name}`}>
      ...
      {(useCase.alternatePaths.length > 0 || useCase.failurePaths.length > 0) && (
        <section className="design-plan-review-section">
          <h3>Alternate and failure paths</h3>
    ...
    <section className="design-plan" aria-label="Plan">
      ...
      <section className="design-plan-sources" aria-label="Analysis sources">
        <div>
          ...
          <p>Read-only inputs used to infer this version. Required failures block approval; optional failures remain visible warnings.</p>
        </div>
            ...
            {state.sourcePreview.status === 'ready' && (
              <>
                ...
                <details className="design-technical-details">
                  ...
                  {state.sourcePreview.truncated && <p>The preview is truncated to 1 MB; the hash covers the complete original.</p>}
                </details>
              </>
            )}
          </section>
