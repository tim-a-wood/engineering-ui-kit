    ...
    <section className="design-waves" aria-label="Implementation plan">
      <div className="design-waves-heading">
        <div>
          ...
          <h2>Plan module builds</h2>
          <p>Dependency waves show the safe order. Choose a module in the handoff workspace below. This plan does not dispatch work.</p>
        </div>
      ...
      {props.mode !== 'project' && (
        <details className="design-waves-multi-select">
          <summary>Multi-module handoff</summary>
          ...
          <div className="design-waves-module-checks">
            {props.wavePlan.waves.flatMap((wave) => wave.modules).map((entry) => (
              <label key={entry.moduleId}>
                <input type="checkbox" checked={selected.has(entry.moduleId)} onChange={() => toggle(entry.moduleId)} aria-label={`Select ${nameByModuleId.get(entry.moduleId) ?? entry.moduleId}`} />
              ...
              {[...selected].map((moduleId) => (
                <label key={moduleId}>
                  <input
                    ...
                    aria-label={`Confirm isolation for ${nameByModuleId.get(moduleId) ?? moduleId}`}
                  />
                  Confirm resource isolation ({nameByModuleId.get(moduleId) ?? moduleId})
                </label>
              ))}
            </fieldset>
          )}
          ...
          {props.multiModuleHandoff && (
            <div className="design-waves-multi-result" role="status" aria-live="polite">
              {props.multiModuleHandoff.result.ok ? (
                ...
                <ul className="design-error-summary" aria-label="Handoff diagnostics">
