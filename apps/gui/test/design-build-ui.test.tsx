...
describe('BuildHandoffView — one-module implementation handoff (§11.2, §11.3, §6.2)', () => {
  ...
  it('a blocked handoff shows the verbatim Build gate blocking reason in completeBaseline mode', () => {
    ...
    const reasons = within(buildHandoffSection()).getByRole('list', { name: 'Build blockers' })
...
describe('WavesView — progressive implementation-wave plan (§11.8)', () => {
  ...
  it('selecting two independent adapters enables the combined multi-module handoff action', () => {
    ...
    fireEvent.click(screen.getByText('Multi-module handoff'))
    ...
    fireEvent.click(screen.getByRole('checkbox', { name: 'Select File-system adapter' }))
    fireEvent.click(screen.getByRole('checkbox', { name: 'Select Git adapter' }))
    ...
    fireEvent.click(screen.getByRole('checkbox', { name: 'Confirm module independence' }))
    fireEvent.click(screen.getByRole('checkbox', { name: 'Confirm combined support' }))
    fireEvent.click(screen.getByRole('checkbox', { name: 'Confirm isolation for File-system adapter' }))
    fireEvent.click(screen.getByRole('checkbox', { name: 'Confirm isolation for Git adapter' }))
  ...
  it('an overlapping-owned-paths selection shows the diagnostic and creates no packet', () => {
    ...
    const diagnostics = within(multiResult as HTMLElement).getByRole('list', { name: 'Handoff diagnostics' })
