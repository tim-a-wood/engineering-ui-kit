...
function manifest(moduleId: string): ModuleManifest {
  return {
    ...
    moduleType: 'domain',
    name: moduleId,
    responsibility: 'Manage module records.',
    ownedConcerns: ['Module records'],
    excludedConcerns: ['User interface'],
    providedOperations: [{ operationId: `op.${moduleId}.read`, contractVersion: '1.0.0' }],
    requiredOperations: [],
    verificationSuiteIds: [`accept.${moduleId}`],
