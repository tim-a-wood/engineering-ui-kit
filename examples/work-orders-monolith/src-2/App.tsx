...
const keyboardGo = (hash: string) => (event: KeyboardEvent<HTMLTableRowElement>) => {
  ...
  const nav = (
    <nav className="mainnav" aria-label="Primary">
      {([
        ...
        const active = route.page === key || (key === 'orders' && route.page === 'order')
        return (
          <a
            key={key}
            href={`#/${key}`}
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand">
          <span className="brand-icon" aria-hidden="true">{ic.wrench}</span>
          <div>
            <h1>PlantOps Maintenance</h1>
            <p>Engineering workbench</p>
          </div>
        </div>
        {nav}
        <div className="sidebar-meta">
          <span>Active site</span>
          <strong>{state.settings.siteName}</strong>
        </div>
      </aside>
      <div className="workspace">
        <header className="topbar">
          <span className="eyebrow">Plant operations</span>
        ...
        <footer className="footer">PlantOps v3.2.1 — internal maintenance tracking. For urgent failures call the control room (ext. 210).</footer>
      </div>
    </div>
  )
}
    ...
    <>
      <header className="page-header">
        <div>
          ...
          <h2>Dashboard</h2>
          <p>Work order load, due risk, and asset health for the active plant.</p>
        </div>
      </header>
      <div className="kpis">
      ...
      {(critical.length > 0 || downAssets.length > 0) && (
        <div className="box alert-panel" role="status" aria-live="polite">
          <strong className="alert-title"><span aria-hidden="true">{ic.alert}</span> Attention</strong>
      ...
      <div className="box">
        <h3 className="panel-title">Recent work orders</h3>
          ...
          <tbody>
            {orders.slice(0, 6).map((o) => (
              <tr
                ...
                onKeyDown={keyboardGo(`#/orders/${o.id}`)}
              >
      ...
      <div className="box">
        <div className="formrow"><label htmlFor="wo-title">Title *</label>
          <input
            ...
            aria-describedby={error ? 'wo-title-error' : undefined}
        ...
        {error && <p id="wo-title-error" className="validation-error" role="alert">{error}</p>}
        <div className="actions">
          ...
          {existing && <button className="danger push" onClick={remove}>Delete</button>}
        </div>
      </div>
    </>
  )
  ...
  return (
    <>
      ...
      <div className="chartrow">
        <div className="chart">
          ...
          <svg width="320" height="150" viewBox="0 0 320 150" role="img" aria-label="Bar chart of work orders by status">
            {byStatus.map((b, i) => (
              <g key={b.status}>
                <rect className={`chart-bar chart-bar-${b.status}`} x={20 + i * 75} y={120 - (b.count / max) * 95} width="44" height={(b.count / max) * 95} />
            ...
            <line className="chart-axis" x1="15" y1="120" x2="315" y2="120" />
          </svg>
        </div>
        <div className="chart">
          ...
          <svg width="320" height="150" viewBox="0 0 320 150" role="img" aria-label="Line chart of completions per week">
            <polyline className="chart-line" points={points} fill="none" strokeWidth="2" />
            {weekly.map((v, i) => (
              <circle className="chart-point" key={i} cx={20 + i * 52} cy={120 - (v / maxW) * 95} r="3" />
            ))}
            <line className="chart-axis" x1="15" y1="120" x2="315" y2="120" />
      ...
      <div className="box">
        ...
        <table className="grid totals-table">
...
function SettingsPage(props: { state: AppState; update: (m: (d: AppState) => AppState) => void }) {
  ...
  return (
    <>
      <h2>Settings</h2>
      <div className="box settings-panel">
