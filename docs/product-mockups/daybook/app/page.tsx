        ...
        >
          <section className="planner-shell" aria-label="Daybook teacher planning studio">
            ...
            <div className="workspace">
              ...
              {section === "Reflections" && <ReflectionsWorkspace onOpenLesson={chooseLesson} onToast={showToast} />}
            </div>
            {teachingOpen && <TeachingView lesson={lesson} step={selectedPhase} sampleIndex={lessonSampleIndex} onStep={setSelectedPhase} onClose={() => setTeachingOpen(false)} onToast={showToast} />}
          </section>
        </div>
      </div>
...
function TeachingView({ lesson, step, sampleIndex, onStep, onClose, onToast }: { lesson: Lesson; step: number; sampleIndex: number; onStep: (step: number) => void; onClose: () => void; onToast: ToastFn }) {
  ...
  return <div className="teaching-overlay" role="dialog" aria-modal="true" aria-label={`Teaching view for ${lesson.title}`}>
    <header className="teaching-topbar">
      ...
      <div className="teaching-top-actions"><button onClick={() => onToast("Child-facing room display opened on the connected screen.")}>▣ Room display</button><button className="end-view" onClick={onClose}>Exit teaching view</button></div>
    </header>
    ...
    <div className="teaching-workspace">
      ...
      <main className="teaching-stage">
        ...
        <nav className="teaching-navigation" aria-label="Teaching phase navigation">
          ...
          {step < lesson.phases.length - 1 ? <button className="next-step" onClick={() => moveStep(step + 1)}>Next: {lesson.phases[step + 1].name} →</button> : <button className="finish-lesson" onClick={() => onToast("Lesson reflection started with this plan attached.")}>Finish & reflect →</button>}
        </nav>
      </main>

      <aside className="teaching-support">
        <section className="teaching-timer">
          <div><span>Phase timer</span><small>{running ? "Running" : remaining === 0 ? "Time" : "Ready"}</small></div>
          <strong>{minutes}<i>:</i>{seconds}</strong>
          <div className="timer-progress"><span style={{ width: `${progress * 100}%` }} /></div>
          <div className="timer-actions"><button onClick={() => { setRunning(false); setRemaining(phase.minutes * 60); }}>Reset</button><button className="timer-main" onClick={() => { if (remaining === 0) { setRemaining(120); setRunning(true); } else { setRunning((current) => !current); } }}>{running ? "Pause" : remaining === 0 ? "Add 2 min" : "Start timer"}</button></div>
        </section>
        ...
        <button className="quick-note" onClick={() => onToast("Quick note captured against this phase.")}>✎ Add a quick note</button>
      </aside>
    </div>
  </div>;
}
...
function ReflectionsWorkspace({ onOpenLesson, onToast }: { onOpenLesson: (id: string) => void; onToast: ToastFn }) {
