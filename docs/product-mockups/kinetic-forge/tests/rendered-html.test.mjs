...
test("binds the detailed GLB hauler to the incident recovery sequence", async () => {
  ...
  assert.match(scene, /craneTrolley/);
  assert.match(scene, /craneCables/);
  assert.match(scene, /clampRig/);
  assert.match(scene, /clampJaws/);
  assert.match(scene, /cageLatches/);
  assert.match(scene, /socketLocks/);
  assert.match(scene, /coreAssembly\.position\.copy\(coreWorld\)/);
  assert.match(scene, /rover\.worldToLocal\(coreAssembly\.position\)/);
  ...
  assert.match(page, /AUTHORIZE CRANE HANDOFF/);
