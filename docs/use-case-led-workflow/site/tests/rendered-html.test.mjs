...
test("uses direct task labels in the briefing and mockup", async () => {
  ...
  assert.match(mockup, /rgba\(4, 9, 17, \.84\)/);
...
test("uses one UML 2.5.1 semantic symbology library", async () => {
  ...
  assert.match(mockup, /const UML_CONNECTORS = Object\.freeze/);
  assert.match(mockup, /function umlConnectorLabel\(/);
  assert.match(mockup, /points:Object\.freeze\(\[\[121,138\],\[280,97\]\]\)/);
  assert.match(mockup, /points:Object\.freeze\(\[\[690,97\],\[805,138\]\]\)/);
  assert.match(mockup, /points:Object\.freeze\(\[\[468,197\],\[484,197\],\[484,97\],\[502,97\]\]\)/);
  assert.match(mockup, /points:Object\.freeze\(\[\[596,162\],\[596,132\]\]\)/);
  assert.match(mockup, /points:Object\.freeze\(\[\[115,102\],\[328,102\]\]\)/);
  assert.match(mockup, /label:Object\.freeze\(\{x:222,y:86\}\)/);
  assert.match(mockup, /label:Object\.freeze\(\{x:376,y:228\}\)/);
  assert.match(mockup, /Object\.values\(UML_CONNECTORS\[type\] \|\| \{\}\)/);
  assert.match(mockup, /ctx\.lineCap = 'butt'/);
  assert.match(mockup, /ctx\.lineJoin = 'miter'/);
