...
| DEV-05 | §13 / §17.2 | `configureBinding`, `verifyConnection`, and `runScenario` execute real work only when the embedding product supplies executors; the adapters return an honest `not-configured` diagnostic, and the record/state layer plus legacy Connect machinery carry the real product path. | A launched deployable or browser runner cannot be conjured from a repository path; faking success would violate the "do not infer completion" rule. Post-review, `applyDelta`, `verifyModule`, and repository context DO execute against the configured project repository. |
| DEV-07 | §17 / §22.1 | In the GUI's project mode, diagram discussion and the Verify and Evidence panels are gated with explicit "not available in project mode yet" notes instead of routing through the bridge; the sample mode retains full behavior. The project-mode actor is `user:local` pending a real identity concept, so approvals require that actor to be configured in the project's roles. | The sample-only backing data for Verify/Evidence is non-canonical and populating it for a real project would fabricate evidence; visible gating was chosen over silent simulation. Diagram-discussion bridge wiring is the largest remaining parity gap and is recorded, not hidden. |
