...
| --- | --- | --- | --- |
| DEV-01 | §17.2 / EUC-05 | RESOLVED (second remediation round). Contracts now have a real persisted approval lifecycle: `analyzeModuleDesign` registers draft contract versions, `approveModuleDesign` approves the module's provided versions with provider review, a changed version cannot be re-approved until every known consumer re-analyzes against it, and packet creation requires persisted approved versions. The derived registry survives only as a documented non-blocking preview for wave planning and next-action hints. | The original judgment that §17.2 forced a derived registry was reviewed as a shortcut; the persisted lifecycle fits inside the existing §17.2 operations without adding new ones. |
| DEV-02 | §17.3 | RESOLVED (second remediation round). The full `DesignOperationResult` (value included) is persisted per project + operation + idempotency key, first-write-wins, so replay after restart is byte-equal; audit events deduplicate on the same scope. | The original reconstruction-from-audit approach was reviewed as insufficient; full-result persistence was implemented. |
...
| DEV-04 | §21 | NARROWED (second remediation round). A repeatable measurement harness (`design-perf-measurement.test.ts`) measures the five §21 targets on the 17-module sample each run, writing run-stamped evidence; all targets currently pass with wide margins (worst median 3.1 ms vs a 100 ms target). A target miss records a visible performance issue entry rather than being silent. Remaining deviation: "reference hardware" is this container, not a controlled hardware program. | §21's "shall measure" is satisfied by the harness; the numeric targets themselves are "should" recommendations. |
| DEV-05 | §13 / §17.2 | NARROWED (second remediation round). `configureBinding`, `verifyConnection` (cli and localhost http kinds), and `runScenario` now have real production executors wired into IPC, machine API, and CLI whenever a project repository is configured: bindings validate against approved contracts and persist; connections launch the module's configured commands and probe the real entry point; scenarios execute step-mapped configured commands with per-step evidence and honest skips. Remaining deviation: ui/schedule/embedded-library binding kinds are rejected as unsupported, the step-to-command mapping and http `localBaseUrl` are documented adapter conventions pending schema fields, and `deadlineAt` is not yet honored mid-command. | A real browser/Electron entry-point harness for ui bindings and schema formalization of the conventions are follow-up work; everything implemented executes real processes and never fabricates success. |
| DEV-07 | §17 / §22.1 | NARROWED (second remediation round). Project mode now has a Setup tab (repository root, session principal, §4 role grants through adapter operations), authority-blocked approvals link to setup, diagram discussion and change plans route through the real §17.2 operations, and Verify renders real scenario coverage with honest empty states. The desktop IPC stamps an OS-derived principal; the GUI asserts no actor. Remaining deviation: the Evidence Explorer stays sample-only (its backing data is sample-specific), Verify detail drill-down covers only the reported first failed step (no list-all-runs operation exists), and the GUI grants the full authority list rather than per-role selection. | The remaining gaps have no honest data source or would require new §17 operations; they are visible in the UI, not simulated. |
...
| 2026-07-25 | External review remediation | independent review (10 P1, 2 P2) → commits `afa51bf`, `e36e125`, `9604e28`, `996598f`, `259d7ce` | All P1s fixed: actor auth + default-deny role authority; project/operation-scoped idempotency with persisted byte-equal replay; persisted contract lifecycle with consumer review; identifier path containment; forbidden-path precedence; blocking rejection of incomplete/out-of-scope deltas; hierarchical owned-path overlap + explicit multi-module confirmations; canonical context priority clamp; repository-rooted desktop executors with aligned inspect/apply revisions; GUI bridge routing with sample mode only when unconfigured. P2s fixed: DEV-06 catalog conflict recorded; evidence runtime environments, immutable run dirs, S-scenario mapping, S26 honesty note. Residual gaps recorded as DEV-07. |
| 2026-07-25 | Second external review remediation | independent review of `05b9604` (8 P1, 3 P2) → commits `111fe41`, `e01657c`, `f2b37f7`, `b492e8d`, `df4b380`, `e1ca234` | All P1s fixed: symlink containment for every product-written path incl. ownership manifest, backups, and rollback; case-insensitive non-human actor denial across all public approval functions with trusted principal stamping at the IPC boundary and embedder principals for machine API/CLI; hardened consumer acks (human authority, real contract, real consumer relationship, revision binding); audit dedup scoped to project+operation+key; record-change kind allowlist with approval-setting rejection and transactional draft persistence in apply; hierarchical owned-path overlap in single-module and Build gates; live-project GUI completion (setup, roles, diagram discussion, Verify); real Connect/scenario executors. P2s fixed: per-execution immutable evidence runs with screenshot hashes and revisions; macOS realpath assertions; ledger DEV rows rewritten to match code (DEV-01/02 resolved, DEV-04/05/07 narrowed). §21 measurement harness added; all five targets met. |
